...
Define persistence, ingestion, and similarity search for **multiple content sources** (corpora) stored as chunks with pgvector embeddings, to support RAG in the chat pipeline. The **CV** is the first source; additional sources MAY be added without re-indexing unrelated corpora when using **incremental** ingestion.
