...
export function CvPdfDownloadButton({
  ...
  return (
    <button
      ...
      onClick={() => void onClick()}
      disabled={busy}
      className="header-btn-primary inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-primary px-0 text-sm font-semibold text-primary-foreground shadow-sm md:w-auto md:px-4 disabled:opacity-60"
      aria-label={downloadLabel}
    >
      <Download className="h-5 w-5 shrink-0 md:hidden" aria-hidden />
